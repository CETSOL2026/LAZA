...
import {
  ...
  FileArchive, FileJson, FileSpreadsheet, FileText, Fuel, Landmark, Search, TrendingUp, Users, WalletCards,
  type LucideIcon,
} from 'lucide-react';
import { loadSourceDownloadCatalog, SourceDownloadAsset, sourceDownloadUrl } from '../data/sourceDownloads';
...
export function SourceFilesMarketplace() {
  ...
  return <div className="space-y-8">
    ...
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-900"><div className="flex items-start gap-3"><FileArchive className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-medium">Governed local source archive</p><p className="mt-1 text-emerald-800/80">Every download is served from LAZA's governed local archive and linked to its official Bronze source asset. Files retain source organisation, publication date, size and SHA-256 evidence; no remote source is contacted when a user clicks Download.</p></div></div></div>

    <div className="flex flex-col gap-3 xl:flex-row xl:items-center"><div className="relative min-w-0 flex-1"><Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search official datasets or sources..." className="w-full rounded-xl border border-border bg-white py-3.5 pl-12 pr-4 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/10" /></div>
      <div className="flex flex-wrap gap-2">{categories.map((item) => <button type="button" key={item} onClick={() => setCategory(item)} className={`rounded-xl px-4 py-3 text-sm transition-colors ${category === item ? 'bg-primary text-white' : 'bg-muted text-foreground hover:bg-muted/70'}`}>{item}</button>)}</div></div>
    ...
    {status === 'loaded' && <div className="grid items-start gap-6 lg:grid-cols-2">{datasets.map(({ pipelineCode, definition, files, primary }) => { const Icon = definition.icon; const isExpanded = expanded === pipelineCode; const formats = [...new Set(files.map((file) => file.assetType))].join(', '); return <article key={pipelineCode} className="overflow-hidden rounded-2xl border border-border bg-white shadow-sm transition-shadow hover:shadow-md">
      <div className="p-6"><div className="flex items-start justify-between gap-4"><div className="flex items-start gap-4"><div className="rounded-xl bg-primary/10 p-3 text-primary"><Icon className="h-6 w-6" /></div><div><h2 className="text-xl">{definition.title}</h2><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-md bg-muted px-2 py-1 text-xs text-muted-foreground">{definition.category}</span><span className="rounded-md bg-emerald-50 px-2 py-1 text-xs text-emerald-700">Official source</span><span className="rounded-md bg-blue-50 px-2 py-1 text-xs text-blue-700">{files.length} {files.length === 1 ? 'file' : 'files'}</span></div></div></div></div>
        ...
        <div className="mt-6 flex gap-2"><a href={sourceDownloadUrl(primary.assetId)} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm text-white transition-colors hover:bg-primary/90"><Download className="h-4 w-4" />{files.length === 1 ? 'Download source file' : 'Download latest file'}</a><button type="button" onClick={() => setExpanded(isExpanded ? null : pipelineCode)} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-sm transition-colors hover:bg-muted">{isExpanded ? 'Hide files' : 'View files'}{isExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button></div>
      </div>
      {isExpanded && <div className="border-t border-border bg-slate-50/70"><div className="max-h-80 divide-y divide-border overflow-y-auto">{[...files].reverse().map((file) => <div key={file.assetId} className="flex items-center gap-3 px-5 py-3"><div className="rounded-lg bg-white p-2 text-muted-foreground shadow-sm"><AssetIcon type={file.assetType} /></div><div className="min-w-0 flex-1"><p className="truncate text-sm" title={file.assetName}>{file.assetName}</p><p className="mt-0.5 text-xs text-muted-foreground">{dateLabel(file.publicationDate)} · {bytes(file.contentSizeBytes)} · SHA-256 {file.sha256.slice(0, 12)}…</p></div><a href={sourceDownloadUrl(file.assetId)} title={`Download ${file.assetName}`} className="rounded-lg border border-border bg-white p-2 text-primary transition-colors hover:bg-primary hover:text-white"><Download className="h-4 w-4" /></a></div>)}</div></div>}
    </article>; })}</div>}
